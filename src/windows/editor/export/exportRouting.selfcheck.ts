...
assert(winHw[0] === "prefer-hardware", "Windows Annex-B starts hardware");
assert(winHw.includes("prefer-software"), "Windows has a software fallback");
...
assert(
  planMp4Routes({ annexBVerified: true, forceRgba: false }).join() ===
    "annexb-ffmpeg,rgba-ffmpeg",
  "verified Annex-B is tried before RGBA on every platform",
);
assert(
